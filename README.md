...
3. root에 .htaccess 파일 생성 후 아래 내용 삽입
	...
	2-1. 참조
		...
		- RewriteEngine On
			url을 재구성 하는 방식으로 직접 페이지를 탐색하는 것이 아니라 하나의 데이터로 받아들이는 설정입니다.
		- Options -Indexes
...
PHP는 원래 스네이크기법으로 파일명을 작성하나, 객체지향으로 프로그래밍 하면 카멜기법으로 씀.
라라벨도 카멜기법. 기존의 객체지향 언어가 카멜기법을 써서.

객체지향 프로그래밍을 하면 기초를 구축 하는데엔 시간이 오래 걸리나, 페이지를 새로 만드는 것에 시간이 적게 듬.